 ...
 */
public class SchoolCommuneBusinessBean extends CaseBusinessBean implements
        SchoolCommuneBusiness, CaseBusiness {
    ...
    /*
     * Commented out since it is never used... private Collection
     * getSchoolYears() throws RemoteException { return
     * getSchoolBusiness().findAllSchoolYears(); }
     */
    ...
    public Map getYearClassMap(Collection schoolYears, int schoolID,
            int seasonID, String emptyString, boolean showSubGroups) {
        try {
            ...
            if (schoolYears != null) {
                ...
                while (iter.hasNext()) {
                    ...
                    groups = sb.findSchoolClassesBySchoolAndSeasonAndYear(
                            schoolID, seasonID,
                            ((Integer) year.getPrimaryKey()).intValue(),
                            showSubGroups);
    ...
    public SchoolChoiceBusiness getSchoolChoiceBusiness()
        ...
        return (SchoolChoiceBusiness) IBOLookup.getServiceInstance(
                getIWApplicationContext(), SchoolChoiceBusiness.class);
    }
    ...
    public Map getStudentList(Collection students) throws RemoteException {
        ...
        if (!students.isEmpty()) {
            Collection users = getCommuneUserBusiness().getUsers(
                    this.getUserIDsFromClassMembers(students));
            User user;
    ...
    public boolean isOngoingSeason(int seasonID) throws RemoteException {
        SchoolSeason season = getSchoolBusiness().getSchoolSeason(
                new Integer(seasonID));
        if (season != null) {
            IWTimestamp stamp = new IWTimestamp();
            return stamp.isBetween(new IWTimestamp(season
                    .getSchoolSeasonStart()), new IWTimestamp(season
                    .getSchoolSeasonEnd()));
        }
        return false;
    }

    public boolean isAlreadyInSchool(int userID, int schoolID, int seasonID)
            throws RemoteException {
        try {
            Collection types = getSchoolBusiness().getSchoolTypeHome()
                    .findAllByCategory(
                            getSchoolBusiness().getCategoryElementarySchool()
                                    .getCategory(), false);
            SchoolClassMember member = getSchoolBusiness()
                    .getSchoolClassMemberHome().findByUserAndSchoolAndSeason(
                            userID, schoolID, seasonID, types);
    ...
    public boolean isPlacedAtSchool(int userID, int schoolID) {
        try {
            return (getSchoolBusiness().getSchoolClassMemberHome()
                    .getNumberOfPlacingsAtSchool(userID, schoolID) > 0);
    ...
    public Map getUserMapFromChoices(Collection choices) throws RemoteException {
        ...
        User user;
        Collection students = getCommuneUserBusiness().getUsers(
                getUserIDsFromChoices(choices));
        if (students != null) {
            Iterator iterator = students.iterator();
            while (iterator.hasNext()) {
                user = (User) iterator.next();
    ...
    public Map getUserAddressesMapFromChoices(Collection choices)
            throws RemoteException {
        HashMap coll = new HashMap();

        if (!choices.isEmpty()) {
            Address address;
            Collection students = getCommuneUserBusiness()
                    .getUsersMainAddresses(getUserIDsFromChoices(choices));
            if (students != null) {
                Iterator iterator = students.iterator();
                while (iterator.hasNext()) {
    ...
    public Map getUserAddressesMapFromChoices(IDOQuery query)
            throws RemoteException {
        HashMap coll = new HashMap();

        Address address;
        Collection students = getCommuneUserBusiness().getUsersMainAddresses(
                query);
        if (students != null) {
            Iterator iterator = students.iterator();
            while (iterator.hasNext()) {
    ...
    public Map getUserPhoneMapFromChoicesUserIdPK(Collection choices)
            throws RemoteException {
        HashMap coll = new HashMap();
        if (!choices.isEmpty()) {
            ...
            while (it.hasNext()) {
                ...
                Phone phone = getCommuneUserBusiness().getChildHomePhone(
                        choice.getChildId());
                if (phone != null)
    ...
    public Map getUserAddressMapFromChoicesUserIdPK(Collection choices)
            throws RemoteException {
        HashMap coll = new HashMap();
        if (!choices.isEmpty()) {
            ...
            while (it.hasNext()) {
                choice = (SchoolChoice) it.next();
                address = getCommuneUserBusiness().getUserAddress1(
                        choice.getChildId());
     ...
     * String.valueOf(student.getClassMemberId()); a++; } return userIDs; }
     * return null; }
     */

    public Map getStudentChoices(Collection students, int seasonID)
            throws RemoteException {
        HashMap coll = new HashMap();
        ...
        if (!students.isEmpty()) {
            ...
            while (iter.hasNext()) {
                ...
                choices = getSchoolChoiceBusiness().findByStudentAndSeason(
                        member.getClassMemberId(), seasonID);
    ...
    public boolean hasChosenOtherSchool(Collection choices, int schoolID) {
        if (choices != null && !choices.isEmpty()) {
            ...
            while (iter.hasNext()) {
                ...
                if ((caseStatus.equalsIgnoreCase("PREL")
                        || caseStatus.equalsIgnoreCase("PLAC") || caseStatus
                        .equalsIgnoreCase("FLYT"))
                        && element.getChosenSchoolId() != schoolID)
                        return true;
            }
        }
    ...
    public boolean[] hasSchoolChoices(int userID, int seasonID)
            throws RemoteException {
        boolean[] returnValue = { false, false};
        int numberOfChoices = getSchoolChoiceBusiness()
                .getNumberOfApplicationsForStudents(userID, seasonID);
    ...
    public boolean hasMoveChoiceToOtherSchool(int userID, int schoolID,
            int seasonID) {
        try {
            int numberOfChoices = getSchoolChoiceBusiness()
                    .getSchoolChoiceHome().getMoveChoices(userID, schoolID,
                            seasonID);
    ...
    public boolean hasChoicesForSeason(int userID, int seasonID) {
        try {
            ...
            int numberOfChoices = getSchoolChoiceBusiness()
                    .getSchoolChoiceHome().getChoices(userID, seasonID,
                            statuses);
            if (numberOfChoices > 0) return true;
            return false;
        } catch (RemoteException e) {
    ...
    public boolean hasChoiceToThisSchool(int userID, int schoolID, int seasonID) {
        try {
            String[] statuses = { getCaseStatusDeleted().getStatus(),
                    getCaseStatusInactive().getStatus()};
            int numberOfChoices = getSchoolChoiceBusiness()
                    .getSchoolChoiceHome().getChoices(userID, schoolID,
                            seasonID, statuses);
            if (numberOfChoices > 0) return true;
            return false;
        } catch (RemoteException e) {
    ...
    public int getChosenSchoolID(Collection choices) {
        if (choices != null && !choices.isEmpty()) {
            ...
            while (iter.hasNext()) {
                ...
                String caseStatus = element.getCaseStatus().toString();
                if (caseStatus.equalsIgnoreCase("PREL")
                        || caseStatus.equalsIgnoreCase("PLAC")
                        || caseStatus.equalsIgnoreCase("FLYT"))
    ...
    public SchoolSeason getPreviousSchoolSeason(SchoolSeason schoolSeason)
            throws RemoteException {
        return getPreviousSchoolSeason(((Integer) schoolSeason.getPrimaryKey())
                .intValue());
    }

    public SchoolSeason getPreviousSchoolSeason(int schoolSeasonID)
            throws RemoteException {
        return getSchoolBusiness().findPreviousSchoolSeason(schoolSeasonID);
    }

    public int getPreviousSchoolSeasonID(int schoolSeasonID)
            throws RemoteException {
        SchoolSeason season = getPreviousSchoolSeason(schoolSeasonID);
        if (season != null) { return ((Integer) season.getPrimaryKey())
                .intValue(); }
        return -1;
    }
    ...
    public int getCurrentSchoolSeasonID() throws RemoteException {
        try {
            return ((Integer) getSchoolChoiceBusiness().getCurrentSeason()
    ...
    public SchoolYear getSchoolYear(int age) throws RemoteException {
        Collection schoolYears = getSchoolBusiness().findAllSchoolYearsByAge(
                age);
    ...
    public int getPreviousSchoolYear(int schoolYearID) throws RemoteException {
        SchoolYear year = getSchoolBusiness().getSchoolYear(
                new Integer(schoolYearID));
        if (year != null) {
    ...
    public SchoolYear getNextSchoolYear(SchoolYear schoolYear)
            throws RemoteException {
    ...
    public Collection getPreviousSchoolClasses(School school,
            SchoolSeason schoolSeason, SchoolYear schoolYear)
            throws RemoteException {
        SchoolSeason season = getPreviousSchoolSeason(schoolSeason);
        if (season != null) {
            SchoolYear year = getSchoolYear(schoolYear.getSchoolYearAge() - 1);
            if (year != null) { return getSchoolBusiness()
                    .findSchoolClassesBySchoolAndSeasonAndYear(
                            ((Integer) school.getPrimaryKey()).intValue(),
    ...
    public void finalizeGroup(SchoolClass schoolClass, String subject,
            String body, boolean confirmation) throws RemoteException {
        SchoolChoice choice;
        User student;
        body = TextSoap.findAndReplace(body, "$barn$", "{0}");
        Collection choices = getSchoolChoiceBusiness().getApplicationsInClass(
                schoolClass, confirmation);
        Iterator iter = choices.iterator();
        String newBody;
    ...
    public void importStudentInformationToNewClass(
            SchoolClassMember schoolMember, SchoolSeason previousSeason)
            throws RemoteException {
        SchoolClassMember oldStudentInfo = getSchoolBusiness()
                .findByStudentAndSeason(schoolMember, previousSeason);
    ...
    public void setNeedsSpecialAttention(int studentID, int schoolSeasonID,
            boolean needsAttention) throws RemoteException {
        setNeedsSpecialAttention(getSchoolBusiness().findByStudentAndSeason(
                studentID, schoolSeasonID), needsAttention);
    }

    public void setNeedsSpecialAttention(SchoolClassMember schoolMember,
            SchoolSeason schoolSeason, boolean needsAttention)
            throws RemoteException {
        setNeedsSpecialAttention(getSchoolBusiness().findByStudentAndSeason(
                schoolMember, schoolSeason), needsAttention);
    }
    ...
    public void addSchoolAdministrator(User user) throws RemoteException,
        ...
        Group rootAdminGroup = getCommuneUserBusiness()
                .getRootSchoolAdministratorGroup();
        getUserBusiness().getGroupBusiness().addUser(
                ((Integer) rootAdminGroup.getPrimaryKey()).intValue(), user);
    }
    ...
    public boolean canMarkSchoolClass(SchoolClass schoolClass,
            String propertyName) throws RemoteException {
        if (schoolClass != null) {
            SchoolSeason season = getSchoolBusiness().getSchoolSeason(
                    new Integer(schoolClass.getSchoolSeasonId()));
            IWTimestamp seasonStart = new IWTimestamp(season
                    .getSchoolSeasonStart());
            IWTimestamp timeNow = new IWTimestamp();
            if (timeNow.getYear() == 2002) return true;

            IWPropertyList properties = getIWApplicationContext()
    ...
    public void moveToGroup(int studentID, int schoolClassID,
            int oldSchoolClassID, int schoolYearID) throws RemoteException {
        SchoolClass group = getSchoolBusiness().findSchoolClass(
                new Integer(oldSchoolClassID));
        if (group.getIsSubGroup()) {
            try {
                SchoolClass newGroup = getSchoolBusiness().findSchoolClass(
                        new Integer(schoolClassID));
                SchoolClassMember member = getSchoolBusiness()
                        .getSchoolClassMemberHome()
                        .findByUserAndSchoolAndSeason(studentID,
            ...
            SchoolClassMember classMember = getSchoolBusiness()
                    .findClassMemberInClass(studentID, oldSchoolClassID);
    ...
    private MessageBusiness getMessageBusiness() throws RemoteException {
        return (MessageBusiness) com.idega.business.IBOLookup
    ...
    private CommuneUserBusiness getCommuneUserBusiness() throws RemoteException {
        return (CommuneUserBusiness) com.idega.business.IBOLookup
                .getServiceInstance(getIWApplicationContext(),
                        CommuneUserBusiness.class);
    }

    public CommuneUserBusiness getUserBusiness() throws RemoteException {
        return (CommuneUserBusiness) this
                .getServiceInstance(CommuneUserBusiness.class);
    }
    ...
    public void resetSchoolClassStatus(int schoolClassID)
            throws RemoteException {
        SchoolClass schoolClass = getSchoolBusiness().findSchoolClass(
                new Integer(schoolClassID));
    ...
    public boolean removeSubGroupPlacements(int userID, int schoolID,
            int seasonID) {
        try {
            Collection placements = getSchoolBusiness().findSubGroupPlacements(
    ...
    public SchoolClassMember[] getCurrentMembersWithInvoiceInterval(
        ...
        try {
            final ProviderAccountingPropertiesHome pHome = (ProviderAccountingPropertiesHome) IDOLookup
                    .getHome(ProviderAccountingProperties.class);
            final Collection schoolIds = pHome
                    .findAllIdsByPaymentByInvoice(true);
            final SchoolHome sHome = (SchoolHome) IDOLookup
        ...
        try {
            ...
            return (SchoolClassMember[]) result
                    .toArray(new SchoolClassMember[result.size()]);
        } catch (FinderException fe) {
    ...
    public SchoolClassMember getCurrentSchoolClassMembership(final User user)
            throws RemoteException {
        try {
            final SchoolSeason season = getSchoolChoiceBusiness()
                    .getCurrentSeason();
            final SchoolClassMember student = getSchoolBusiness()
                    .findByStudentAndSeason(user, season);
            return (null == student || null != student.getRemovedDate()) ? null
                    : student;
    ...
    public SchoolClassMember getCurrentSchoolClassMembership(final User user,
        ...
        try {
            ...
            final SchoolClassMember student = getSchoolBusiness()
                    .findByStudentAndSchoolAndSeason(userId, schoolId, seasonId);
            return (null == student || null != student.getRemovedDate()) ? null
                    : student;
        } catch (final FinderException e) {
            return null;
    ...
    public SchoolStudyPath getStudyPath(final SchoolClassMember student) {
        try {
            final SchoolStudyPathHome home = (SchoolStudyPathHome) IDOLookup
                    .getHome(SchoolStudyPath.class);
            final int studyPathId = student.getStudyPathId();
            return 0 < studyPathId ? home.findByPrimaryKey(new Integer(
                    studyPathId)) : null;
        } catch (FinderException e) {
            return null;
    ...
    public SchoolStudyPath[] getAllStudyPaths() {
        ...
        try {
            ...
            if (null != studyPathCollection) {
                result = (SchoolStudyPath[]) studyPathCollection
                        .toArray(new SchoolStudyPath[studyPathCollection.size()]);
            }
            return result;
        } catch (FinderException e) {
            return result;
    ...
    private void initializeBundlesIfNeeded(Locale currentLocale) {
        if (_iwb == null) {
            _iwb = this.getIWApplicationContext().getIWMainApplication()
                    .getBundle(IW_BUNDLE_IDENTIFIER);
        }
        _iwrb = _iwb.getResourceBundle(currentLocale);
    }

    public ReportableCollection getReportOfUsersNotRegisteredInAnyClass(
            Locale currentLocale, Date selectedDate,
            SchoolSeason currentSeason, Collection classes)
            throws IDOException, RemoteException, CreateException,
            FinderException {
        initializeBundlesIfNeeded(currentLocale);
        CommuneUserBusiness _communeUserService = (CommuneUserBusiness) IBOLookup
                .getServiceInstance(this.getIWApplicationContext(),
                        CommuneUserBusiness.class);
        MemberFamilyLogic _familyLogic = (MemberFamilyLogic) IBOLookup
                .getServiceInstance(this.getIWApplicationContext(),
        ...
        SchoolBusiness sBusiness = (SchoolBusiness) IBOLookup
                .getServiceInstance(this.getIWApplicationContext(),
                        SchoolBusiness.class);

        SchoolClassMemberHome scmHome = sBusiness.getSchoolClassMemberHome();

        Collection schoolyears = sBusiness.getSchoolYearHome()
                .findAllSchoolYears();
        IWTimestamp seasonStartDate = new IWTimestamp(currentSeason
                .getSchoolSeasonStart());
        ...
        CitizenHome citizenHome = (CitizenHome) IDOLookup
                .getHome(Citizen.class);
        //maindata
        Collection students = citizenHome
                .findCitizensNotAssignedToClassOnGivenDate(communeGroup,
                        new java.sql.Date(selectedDate.getTime()), classes,
                        firstDateOfBirth.getDate(), lastDateOfBirth.getDate());

        GroupRelationHome gRelationHome = ((GroupRelationHome) IDOLookup
                .getHome(GroupRelation.class));

        //initializing fields
        IDOEntityDefinition userDef = IDOLookup
                .getEntityDefinitionForClass(User.class);
        IDOEntityDefinition grRelDef = IDOLookup
                .getEntityDefinitionForClass(GroupRelation.class);
        IDOEntityDefinition addrDef = IDOLookup
                .getEntityDefinitionForClass(Address.class);
        DateFormat dateFormat = DateFormat.getDateInstance(DateFormat.DEFAULT,
                currentLocale);

        //Child - Fields
        ReportableField childPersonalID = new ReportableField(userDef
                .findFieldByUniqueName(User.FIELD_PERSONAL_ID));
        childPersonalID.setCustomMadeFieldName("child_ssn");
        childPersonalID.setLocalizedName(_iwrb.getLocalizedString(
                "CommuneReportBusiness.child_ssn", "Personal ID"),
                currentLocale);
        reportData.addField(childPersonalID);

        ReportableField childLastName = new ReportableField(userDef
                .findFieldByUniqueName(User.FIELD_LAST_NAME));
        childLastName.setCustomMadeFieldName("child_last_name");
        childLastName.setLocalizedName(_iwrb.getLocalizedString(
                "CommuneReportBusiness.child_last_name", "LastName"),
                currentLocale);
        reportData.addField(childLastName);

        ReportableField childFirstName = new ReportableField(userDef
                .findFieldByUniqueName(User.FIELD_FIRST_NAME));
        childFirstName.setCustomMadeFieldName("child_first_name");
        childFirstName.setLocalizedName(_iwrb.getLocalizedString(
                "CommuneReportBusiness.child_first_name", "FirstName"),
                currentLocale);
        reportData.addField(childFirstName);

        ReportableField childAddress = new ReportableField(addrDef
                .findFieldByUniqueName(Address.FIELD_STREET_NAME));
        childAddress.setValueClass(String.class);
        childAddress.setCustomMadeFieldName("child_address");
        childAddress.setLocalizedName(_iwrb.getLocalizedString(
                "CommuneReportBusiness.child_address", "Address"),
                currentLocale);
        reportData.addField(childAddress);

        ReportableField childGroupInvitationDate = new ReportableField(grRelDef
                .findFieldByUniqueName(GroupRelation.FIELD_INITIATION_DATE));
        childGroupInvitationDate.setValueClass(String.class);
        childGroupInvitationDate
                .setCustomMadeFieldName("child_gr_initiation_date");
        childGroupInvitationDate.setLocalizedName(_iwrb.getLocalizedString(
                "CommuneReportBusiness.child_gr_initiation_date",
                "Invitiation date"), currentLocale);
        reportData.addField(childGroupInvitationDate);

        ReportableField lastPlacementField = new ReportableField(
                "last_placement", String.class);
        lastPlacementField.setLocalizedName(_iwrb.getLocalizedString(
                "SchoolCommuneBusiness.lastPlacement",
                "Last known school or child care / date"), currentLocale);
        reportData.addField(lastPlacementField);

        //Parent1 - Fields
        ReportableField parent1PersonalID = new ReportableField(userDef
                .findFieldByUniqueName(User.FIELD_PERSONAL_ID));
        parent1PersonalID.setCustomMadeFieldName("parent1_ssn");
        parent1PersonalID.setLocalizedName(_iwrb.getLocalizedString(
                "CommuneReportBusiness.parent1_ssn", "Parent1 Personal ID"),
                currentLocale);
        reportData.addField(parent1PersonalID);

        ReportableField parent1LastName = new ReportableField(userDef
                .findFieldByUniqueName(User.FIELD_LAST_NAME));
        parent1LastName.setCustomMadeFieldName("parent1_last_name");
        parent1LastName.setLocalizedName(_iwrb.getLocalizedString(
                "CommuneReportBusiness.parent1_last_name", "Parent1 LastName"),
                currentLocale);
        reportData.addField(parent1LastName);

        ReportableField parent1FirstName = new ReportableField(userDef
                .findFieldByUniqueName(User.FIELD_FIRST_NAME));
        parent1FirstName.setCustomMadeFieldName("parent1_first_name");
        parent1FirstName.setLocalizedName(_iwrb
                .getLocalizedString("CommuneReportBusiness.parent1_first_name",
                        "Parent1 FirstName"), currentLocale);
        reportData.addField(parent1FirstName);

        ReportableField parent1Address = new ReportableField(addrDef
                .findFieldByUniqueName(Address.FIELD_STREET_NAME));
        parent1Address.setValueClass(String.class);
        parent1Address.setCustomMadeFieldName("parent1_address");
        parent1Address.setLocalizedName(_iwrb.getLocalizedString(
                "CommuneReportBusiness.parent1_address", "Parent1 Address"),
                currentLocale);
        reportData.addField(parent1Address);

        ReportableField parent1GroupInvitationDate = new ReportableField(
                grRelDef
                        .findFieldByUniqueName(GroupRelation.FIELD_INITIATION_DATE));
        parent1GroupInvitationDate.setValueClass(String.class);
        parent1GroupInvitationDate
                .setCustomMadeFieldName("parent1_gr_initiation_date");
        parent1GroupInvitationDate.setLocalizedName(_iwrb.getLocalizedString(
                "CommuneReportBusiness.parent1_gr_initiation_date",
                "Parent1 Invitiation date"), currentLocale);
        reportData.addField(parent1GroupInvitationDate);

        //Parent2 - Fields
        ReportableField parent2PersonalID = new ReportableField(userDef
                .findFieldByUniqueName(User.FIELD_PERSONAL_ID));
        parent2PersonalID.setCustomMadeFieldName("parent2_ssn");
        parent2PersonalID.setLocalizedName(_iwrb.getLocalizedString(
                "CommuneReportBusiness.parent2_ssn", "Parent2 Personal ID"),
                currentLocale);
        reportData.addField(parent2PersonalID);

        ReportableField parent2LastName = new ReportableField(userDef
                .findFieldByUniqueName(User.FIELD_LAST_NAME));
        parent2LastName.setCustomMadeFieldName("parent2_last_name");
        parent2LastName.setLocalizedName(_iwrb.getLocalizedString(
                "CommuneReportBusiness.parent2_last_name", "Parent2 LastName"),
                currentLocale);
        reportData.addField(parent2LastName);

        ReportableField parent2FirstName = new ReportableField(userDef
                .findFieldByUniqueName(User.FIELD_FIRST_NAME));
        parent2FirstName.setCustomMadeFieldName("parent2_first_name");
        parent2FirstName.setLocalizedName(_iwrb
                .getLocalizedString("CommuneReportBusiness.parent2_first_name",
                        "Parent2 FirstName"), currentLocale);
        reportData.addField(parent2FirstName);

        ReportableField parent2Address = new ReportableField(addrDef
                .findFieldByUniqueName(Address.FIELD_STREET_NAME));
        parent2Address.setValueClass(String.class);
        parent2Address.setCustomMadeFieldName("parent2_address");
        parent2Address.setLocalizedName(_iwrb.getLocalizedString(
                "CommuneReportBusiness.parent2_address", "Parent2 Address"),
                currentLocale);
        reportData.addField(parent2Address);

        ReportableField parent2GroupInvitationDate = new ReportableField(
                grRelDef
                        .findFieldByUniqueName(GroupRelation.FIELD_INITIATION_DATE));
        parent2GroupInvitationDate.setValueClass(String.class);
        parent2GroupInvitationDate
                .setCustomMadeFieldName("parent2_gr_initiation_date");
        parent2GroupInvitationDate.setLocalizedName(_iwrb.getLocalizedString(
                "CommuneReportBusiness.parent2_gr_initiation_date",
                "Parent2 Invitiation date"), currentLocale);
        reportData.addField(parent2GroupInvitationDate);

        IWTimestamp currentTime = IWTimestamp.RightNow();
        long oneYearAgo = new IWTimestamp(currentTime.getDay(), currentTime
                .getMonth(), currentTime.getYear() - 1).getTimestamp()
                .getTime();
        ...
        while (iter.hasNext()) {
            ...
            Address childAddressEntiy = _communeUserService
                    .getUsersMainAddress(child);
            String childAddressString = this.getAddressString(
                    childAddressEntiy, _iwrb);
            if (childAddressString != null) {
                data.addData(childAddress, childAddressString);
            }

            Collection coll = gRelationHome
                    .findGroupsRelationshipsContainingGroupsAndStatus(
                            communeGroup, child, GroupRelation.STATUS_ACTIVE);
            ...
            if (iterator.hasNext()) {
                ...
                if (time != null) {
                    if (time.getTime() > oneYearAgo) {
                        data.addData(childGroupInvitationDate, dateFormat
                                .format(time));
                    }
                } else {
                ...
                data.addData(childGroupInvitationDate,
                        "No relation to commune specified");
            }
            ...
            try {
                ...
                if (latestSCM != null) {
                    ...
                    data.addData(lastPlacementField, school.getSchoolName()
                            + " " + sc.getSchoolClassName() + " / "
                            + dateFormat.format(latestSCM.getRegisterDate()));
                } else {
                    data.addData(lastPlacementField, _iwrb.getLocalizedString(
            ...
            try {
                ...
                if (parents != null) {
                    ...
                    if (pIter.hasNext()) {
                        ...
                        Address parent1AddressEntiy = _communeUserService
                                .getUsersMainAddress(child);
                        String parent1AddressString = this.getAddressString(
                                parent1AddressEntiy, _iwrb);
                        if (parent1AddressString != null) {
                            data.addData(parent1Address, parent1AddressString);
                        }

                        Collection pColl = gRelationHome
                                .findGroupsRelationshipsContainingGroupsAndStatus(
                                        communeGroup, parent,
                                        GroupRelation.STATUS_ACTIVE);
                        Iterator pIterator = pColl.iterator();
                        while (pIterator.hasNext()) {
                            GroupRelation rel = (GroupRelation) pIterator
                                    .next();
                            Timestamp time = rel.getInitiationDate();
                            if (time != null) {
                                data.addData(parent1GroupInvitationDate,
                                ...
                                data
                                        .addData(
                                                ...
                                                _iwrb
                                                        .getLocalizedString(
                                                                "CommuneReportBusiness.no_time_specified",
                                                                "No time specified"));
                            }
                        }
                    }
                    ...
                    if (pIter.hasNext()) {
                        ...
                        Address parent2AddressEntiy = _communeUserService
                                .getUsersMainAddress(child);
                        String parent2AddressString = this.getAddressString(
                                parent2AddressEntiy, _iwrb);
                        ...
                        if (pIterator.hasNext()) {
                            ...
                            Timestamp time = rel.getInitiationDate();
                            if (time != null) {
                                data.addData(parent2GroupInvitationDate,
                                        dateFormat.format(time));
                            } else {
                                data
                                        .addData(
        ...
        reportData.addExtraHeaderParameter("label_current_date", _iwrb
                .getLocalizedString("CommuneReportBusiness.label_current_date",
                        "Current date"), "current_date", dateTimeFormat
                .format(IWTimestamp.getTimestampRightNow()));

        return reportData;
    }

    public void sendMessageToParents(SchoolChoice choice, String subject,
            String body) {
        try {
            ...
            if (getUserBusiness().getMemberFamilyLogic().isChildInCustodyOf(
                    child, appParent)) {
                getMessageBusiness().createUserMessage(choice, appParent,
                        subject, MessageFormat.format(body, arguments), true);
            }

            try {
                Collection parents = getUserBusiness().getMemberFamilyLogic()
                        .getCustodiansFor(child);
                Iterator iter = parents.iterator();
                while (iter.hasNext()) {
                    User parent = (User) iter.next();
                    if (!getUserBusiness().haveSameAddress(parent, appParent)) {
                        getMessageBusiness().createUserMessage(choice, parent,
                                subject, MessageFormat.format(body, arguments),
                                true);
    ...
    private String getAddressString(Address addressEntiy, IWResourceBundle iwrb) {
        if (addressEntiy != null) {
            ...
            String postalCode = iwrb.getLocalizedString(
                    "CommuneReportBusiness.not_available", "N/A");
            PostalCode pCodeObj = addressEntiy.getPostalCode();
            String city = addressEntiy.getCity();
            if (pCodeObj != null) {
                postalCode = pCodeObj.getPostalCode();
            } else if (city == null) {
                String childAddressString = stName
                        + ((number == null) ? "" : (" " + number));
