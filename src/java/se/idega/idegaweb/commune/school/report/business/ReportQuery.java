/*
 * $Id: ReportQuery.java,v 1.20 2006/04/09 11:39:54 laddi Exp $
 *
...
public class ReportQuery {
	...
	public void setSelectCountPlacements(int schoolSeasonId) {
		this.sql = "select count(*) from ic_user u, ic_address a, ic_user_address ua, sch_class_member cm," +
				" sch_school_class sc, sch_school s, sch_school_year sy where" +
				" cm.register_date <= '" + this.currentDate + 
				"' and (cm. removed_date is null or cm.removed_date > '" + this.currentDate + "')" + 
	...
	public void setSelectCountPlacements(int schoolSeasonId, int schoolId) {
		this.sql = "select count(*) from sch_class_member cm," +
				" sch_school_class sc, sch_school s, sch_school_year sy where" +
				" cm.register_date <= '" + this.currentDate + 
				"' and (cm. removed_date is null or cm.removed_date > '" + this.currentDate + "')" + 
				" and sc.school_id = s.sch_school_id and sc.sch_school_class_id = cm.sch_school_class_id" +
	...
	public void setSelectCountPlacementsSixYearsOld(int schoolSeasonId, int schoolId) {
		this.sql = "select count(*) from ic_user u, sch_class_member cm," +
				" sch_school_class sc, sch_school s, sch_school_year sy where" +
				" cm.register_date <= '" + this.currentDate + 
				"' and (cm. removed_date is null or cm.removed_date > '" + this.currentDate + "')" + 
				" and sc.school_id = s.sch_school_id and sc.sch_school_class_id = cm.sch_school_class_id" +
	...
	public void setOnlyNackaCitizens() {
		this.sql += " and ua.ic_user_id = u.ic_user_id and a.ic_address_id = ua.ic_address_id" +
				" and a.ic_address_type_id = 1 and a.ic_commune_id = 1 and cm.ic_user_id = u.ic_user_id";
	}
	...
	public void setSchoolType(int schoolTypeId) {
		this.sql += " and cm.sch_school_type_id = " + schoolTypeId;
	}
	...
	public void setSchoolTypeElementarySchool() {
		this.sql += " and cm.sch_school_type_id = 4";
	}
	...
	public void setSchoolTypeCompulsorySchool() {
		this.sql += " and cm.sch_school_type_id = 28";
	}
	...
	public void setSchoolTypePreSchoolClass() {
		this.sql += " and cm.sch_school_type_id = 5";
	}
	...
	public void setSchoolTypeHighSchool() {
		this.sql += " and cm.sch_school_type_id = 26";
	}
	...
	public void setSchoolTypeCompulsoryHighSchool() {
		this.sql += " and cm.sch_school_type_id = 27";
	}
	...
	public void setSchoolYear(String schoolYearName) {
		this.sql += " and cm.sch_school_year_id = sy.sch_school_year_id and sy.year_name = '" + schoolYearName + "'";
	}
	...
	public void setOnlyPrivateSchools() {
		this.sql += " and (s.management_type = 'COMPANY' or s.management_type = 'PRIVATE' or s.management_type = 'FOUNDATION' or s.management_type = 'OTHER')";
	}
	...
	public void setNotPrivateSchools() {
		this.sql += " and s.management_type <> 'COMPANY' and s.management_type <> 'PRIVATE' and s.management_type <> 'FOUNDATION' and s.management_type <> 'OTHER'";
	}
	...
	public void setOnlyForieignSchools() {
		this.sql += " and s.school_name = 'Utlandselever'";
	}
	...
	public void setOnlyCountyCouncilSchools() {
		this.sql += " and s.management_type = 'COUNTY COUNCIL'";
	}
	...
	public void setOnlyStudentsBorn(int year) {
		this.sql += " and u.date_of_birth >= '" + year + "-01-01' and u.date_of_birth <= '" + year + "-12-31'";
	}
	...
	public void setSchool(int schoolId) {
		this.sql += " and s.sch_school_id = " + schoolId;
	}
	...
	public void setSchools(Collection schools) {
		this.sql += " and s.sch_school_id in (";
		Iterator iter = schools.iterator();
		while (iter.hasNext()) {
			School school = (School) iter.next();
			this.sql += school.getPrimaryKey();
			if (iter.hasNext()) {
				this.sql += ", ";
			}
		}
	...
	public void setStudentAge(SchoolSeason season, int ageFrom, int ageTo) {
		...
		if (ageFrom > 0) {
			...
			this.sql += " and u.date_of_birth <= '" + dateFrom + "'";
		}
		if (ageTo > 0) {
			dateTo = "" + (seasonYear - ageTo - 1) + seasonDate.substring(4);
			this.sql += " and u.date_of_birth > '" + dateTo + "'";
		}
	}
	...
	public int execute() {
		...
		try {
			...
			System.out.println("ReportQuery.execute() exception, sql = " + this.sql);
